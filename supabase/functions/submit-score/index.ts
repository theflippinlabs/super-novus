...
// redeploy: v6 — no wallet signature required (see SECURITY NOTE).
// Validates + rate-limits a submission and upserts the wallet's best score into
// the current WEEKLY and MONTHLY periods.
...
//
// SECURITY NOTE (documented honestly): the score is computed client-side and this
// function does NOT require a wallet signature. Earlier versions demanded an
// EIP-191 personal_sign, but on mobile WalletConnect that prompt frequently never
// surfaced inside the wallet — making it impossible for real players to save a
// score. Since a client-computed score was never strong anti-cheat anyway, we drop
// the signature and rely on: a valid wallet address, value bounds, a ±5-min
// timestamp window, and per-wallet rate limiting. A deliberate, documented
// trade-off: a working leaderboard over signature theatre.
...
Deno.serve(async (req) => {
  ...
  const { wallet, score, dist, dust, ts } = body ?? {};
  // Big Bang count is informational; clamp it.
  ...
  //    A score is submitted by the connected wallet address. We intentionally do
  //    NOT demand a personal_sign, because on mobile WalletConnect the sign prompt
  //    frequently never surfaces inside the wallet, which made saving a score
  //    impossible for real users. The score is computed client-side anyway (this
  //    was never strong anti-cheat), so authenticity relies on: a valid wallet
  //    address, value bounds (above), a ±5-min timestamp window (above), and
  //    per-wallet rate limiting (below). This is a deliberate, documented trade-off
  //    favouring "the leaderboard actually works" over signature theatre.
